...
class PuzzleState:
    ...
    def is_goal(self):
        goal = [[(i * self.size + j) for j in range(self.size)] for i in range(self.size)]
...
def is_solvable(state):
    ...
    if is_even:
        '''
        If even board then
        1) zero is on an even row (from the bottom)-> inversion has to be odd for solvability
        2) zero is on an odd row (from the bottom) -> inversion has to be even for solvability
        '''
        # zero on odd row
        if (size - state.find_zero_position()[0]) % 2 == 1:
    ...
    while unvisited_list:
        ...
        for neighbor in current_state.get_neighbors():
            if tuple(map(tuple, neighbor.board)) in visited_set:
                continue
            unvisited_list.append(neighbor)
